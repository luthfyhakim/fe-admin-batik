      ...
      <div class="lg:ml-64">
        ...
        <main class="pt-20 min-h-screen">
          <div id="bg-2" class="p-4">
            <h1 class="text-center text-4xl font-bold text-white pe-4 ps-4 pb-4">Data Kategori</h1>
            ...
            <!-- Add New kategori Modal -->
            <Modal v-if="showAddModal" @close="showAddModal = false">
              <form @submit.prevent="handleSubmit" class="space-y-4">
                <h1 class="text-center text-xl font-bold text-white pe-4 ps-4 pb-4">Add New Data Kategori</h1>
                <div class="grid grid-cols-1 gap-4">
                  <input v-model="formData.nama_kategori" type="text" placeholder="Nama Kategori" class="p-2 rounded-lg bg-white/10 border border-white/20 text-white placeholder-white/50 focus:outline-none focus:ring-2 focus:ring-white/20" required>
                </div>
            ...
            <Modal v-if="showEditModal" @close="showEditModal = false">
              <form @submit.prevent="handleEdit" class="space-y-4">
                <h1 class="text-center text-xl font-bold text-white pe-4 ps-4 pb-4">Edit Data kategori</h1>
                <div class="grid grid-cols-1 gap-4">
                  <input v-model="editData.nama_kategori" type="text" placeholder="Nama Kategori" class="p-2 rounded-lg bg-white/10 border border-white/20 text-white placeholder-white/50 focus:outline-none focus:ring-2 focus:ring-white/20" required>
                </div>
                <div class="flex justify-end space-x-2">
            ...
            <div class="block p-6 bg-gray-500 rounded-xl bg-clip-padding backdrop-filter backdrop-blur-lg bg-opacity-20 shadow-xl text-white mt-8">
              ...
              <!-- Table -->
                ...
                <table class="min-w-full bg-gray-800 text-white rounded-lg shadow-lg overflow-hidden">
                    ...
                    <tbody>
                    <tr v-for="(item, index) in filteredkategoris" :key="item.id_kategori" class="border-b border-gray-600">
                        ...
                        <button @click="handleDelete(item.id_kategori)" type="button" class="px-4 py-2 text-white rounded-lg transition m-1" id="button-2">Delete</button>
                        </td>
                    </tr>
                    <!-- No Results Message -->
...
// Computed property for filtered products
    const filteredkategoris = computed(() => {
      if (!searchQuery.value) return kategoris.value;
      
      const query = searchQuery.value.toLowerCase();
      return kategoris.value.filter(product => 
    ...
    const handleSearch = debounce(() => {
      ...
      const openEditModal = (kategori) => {
        editData.value = { ...kategori };
        showEditModal.value = true;
      };
  
      const handleEdit = async () => {
        try {
          const response = await fetch(`http://localhost:8080/kategori/${editData.value.id_kategori}`, {
            method: 'PATCH',
            headers: {
              'Content-Type': 'application/json',
            },
            body: JSON.stringify({
              nama_kategori: editData.value.nama_kategori
            })
          });
      ...
      const handleDelete = async (id) => {
        if (confirm('Are you sure you want to delete this kategori?')) {
          try {
            const response = await fetch(`http://localhost:8080/kategori/${id}`, {
              method: 'DELETE'
            });
